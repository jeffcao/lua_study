...
-- 单张扑克牌定义
function PokeCard:ctor(parent, filename, scaleFactor)
	...
	self.card_sprite = CCSprite:createWithSpriteFrameName(self.image_filename)
	self.card_sprite:setAnchorPoint(ccp(0,0))
	self.card_sprite:setPosition(ccp(-150, -150))
	self.card_sprite:setVisible(false)
	parent:addChild(self.card_sprite)
...
PokeCard.resetAll = function(parent)
	PokeCard.releaseAllCards()
	PokeCard.sharedPokeCard(parent)
end
