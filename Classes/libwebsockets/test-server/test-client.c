					       ...
					       void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_CLIENT_RECEIVE:
		...
		fprintf(stderr, "rx %d '%s'\n", (int)len, (char *)in);
            if (strncmp((char*)in, "[[\"websocket_rails.ping\"", 24) == 0) {
                fprintf(stderr, "send pong back\n");
                char buf[LWS_SEND_BUFFER_PRE_PADDING + 27 + LWS_SEND_BUFFER_POST_PADDING];
                strncpy(&buf[LWS_SEND_BUFFER_PRE_PADDING], "[\"websocket_rails.pong\",{}]", 27);
                libwebsocket_write(wsi, (unsigned char *) &buf[LWS_SEND_BUFFER_PRE_PADDING], 27, LWS_WRITE_TEXT);
...
int main(int argc, char **argv)
{
	...
	wsi_dumb = libwebsocket_client_connect(context, address, port, use_ssl,
			"/websocket", argv[optind], argv[optind],
	...
	while (n >= 0 && !was_closed && !force_exit) {
        ...
        fprintf(stderr, "start to connect in mirror...\n");

		/* create a client websocket using mirror protocol */

		wsi_mirror = libwebsocket_client_connect(context,
			address, port, use_ssl,  "/websocket",
