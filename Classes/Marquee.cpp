...
void Marquee::init(cocos2d::CCLayer* parent, float x, float y) {
    ...
    rectangle[2] = ccp(this->_clip_node->getContentSize().width, this->_clip_node->getContentSize().height);
    ...
    CCLabelTTF *content = CCLabelTTF::create (str, this->_font, this->_text_size, CCSizeMake (this->_size.width, 0), kCCTextAlignmentLeft);
    CCLabelTTF *content2 = CCLabelTTF::create (str, this->_font, this->_text_size, CCSizeMake (this->_size.width, 0), kCCTextAlignmentLeft);
    ...
    content->setPosition( ccp(this->_clip_node->getContentSize().width / 2, this->_clip_node->getContentSize().height) );
    this->_clip_node->addChild(content);
    content->runAction(CCRepeatForever::create(CCSequence::create(
...
void Marquee::before_callback(CCNode* pSender, void* data) {
	...
	CCLabelTTF* pSucc = static_cast<CCLabelTTF*>(pSender->getParent()->getChildByTag(tag));
...
void Marquee::after_callback(CCNode* pSender, void* data) {
	...
	if (s_count == 0) {
		pSender->setPosition( ccp(pSender->getParent()->getContentSize().width / 2, 0) );
