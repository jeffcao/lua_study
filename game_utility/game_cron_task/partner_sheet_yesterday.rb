...
date = (Time.now.to_date() -1).strftime("%Y-%m-%d").to_s
...
one_day_date  = 2.day.ago(Time.now).strftime("%Y-%m-%d")
three_day_date  = 4.days.ago(Time.now).strftime("%Y-%m-%d")
seven_day_date  = 8.days.ago(Time.now).strftime("%Y-%m-%d")
appids.each do |appid|
  ...
  if appid.to_s.length>1
    email = Partmer.find_by_appid(appid).email unless Partmer.find_by_appid(appid).nil?
  end
