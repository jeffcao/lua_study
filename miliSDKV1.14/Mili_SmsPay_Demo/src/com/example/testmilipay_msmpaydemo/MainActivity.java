...
public class MainActivity extends Activity {
	private Button pay1;
	private String payId = "51326907";// 计费项ID，联系商务
	...
	private void dopay() {
		...
		// orderNum为订单号，为了保证唯一性，各位开发者最好在后台生成唯一性的订单号。不要在前端生成。
		...
		dreamPay.Pay(new PayCallback() {
			...
			public void pay(int payResult) {
				// 开发者在这里不要所有的错误码都提示支付失败，最好提示的比较合理，让用户进行尝试，提高支付成功率。比如提示4004
				// 40041 40045
				// 都是网络异常相关的异常，开发者可以考虑设计一个重试的页面。
				if (payResult == 9000) {
					// 成功
					Toast.makeText(MainActivity.this,
...
//							.show();
					...
					Toast.makeText(MainActivity.this,
							"支付异常，请重新尝试！", Toast.LENGTH_LONG)
							.show();
				}
				// 这里开发者可以自己进行统计，把订单号带上。方便查询。
			}
		}, payId, checkBox.isChecked(), orderNum);
	}
